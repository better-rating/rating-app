<template>
    <svg class="vue-star-rating-star" :height="getSize" :width="getSize" :viewBox="viewBox" @mousemove="mouseMoving" @click="selected">
        ...
        <polygon v-if="!reset" :points="starPointsToString" :fill="getGradId" :stroke="glowColor"
              :filter="'url(#'+this.glowId+')'" v-show="fill > 1"  :stroke-dasharray="reset ? '7 4' : '1 0'"/>

        <polygon :points="starPointsToString" :fill="getGradId" :stroke="getBorderColor" :stroke-width="border" :stroke-linejoin="roundedCorners ? 'round' : 'miter'" :stroke-dasharray="reset ? '7 4' : '1 0'"/>
        <polygon :points="starPointsToString" :fill="reset ? '#f8fafc' : getGradId" />
    </svg>
</template>
